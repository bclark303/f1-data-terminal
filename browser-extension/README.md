...
4. For replays, return to the standalone terminal when needed. The companion first reads Bitmovin's own player timeline from the seekbar accessibility state; F1 TV can place the underlying HTML5 media element on a different MediaSource timestamp origin. It then tries the media UTC clock, and finally the curated race-start offset. Open AUTO/SYNC to see the player clock, raw media clock, and selected sync method. MATCH NOW remains the last-resort fallback.
