...
Open **/live** for the standalone live terminal. It connects through the local Next server to Formula 1's anonymous SignalR Core timing stream and subscribes to a broad, best-effort superset of currently observed timing topics. Alongside the core DriverList, TimingData/TimingDataF1, TimingAppData, TimingStats, RaceControlMessages, WeatherData, TrackStatus, LapCount, SessionInfo/Status, ExtrapolatedClock, CarData.z and Position.z feeds, it can consume optional feeds for current tyres and stint series, pit-stop and pit-lane timing, lap-position series, overtakes, team radio, driver race information/scores, TopThree, live championship projections, weather series, audio/content streams and related race-control data when Formula 1 publishes them.

The live UI is driver-centric: selecting a car carries its team colour and identity through timing, telemetry, track position, strategy/activity, race-control mentions and team radio. A sticky **driver rail** keeps position, identity and tyre age visible while moving between panels. The **Race Story** panel derives explainable selected-driver context—such as pace versus personal best, catching/battle state, tyre age, speed-trap rank and position trend—while the **Commentary Desk** turns session-wide live facts into short, explicitly derived notes for close battles, overtakes, biggest movers, pit activity, fastest laps and changing track/weather states. These notes are descriptive rather than predictive.

Session statistic tiles surface fastest lap, top speed, closest interval and oldest stint. F1-supplied championship projections are identified as projections rather than treated as results. Race-control entries are linked back to any driver numbers/names they mention, and radio clips keep the matching driver identity attached. A feed-coverage panel shows which subscribed topics have actually arrived so unavailable data is visible instead of silently inferred. The **Feed / Driver Explorer** makes every received subscribed payload inspectable and extracts scalar values tied to the selected car from optional feeds such as TimingStats, DriverRaceInfo, DriverScore, DriverTracker, TopThree, tyre series and championship prediction. Raw car-data channel 45 remains labelled **Aero ch45** because its meaning varies by ruleset and feed version.

Team-radio transcription is optional. F1-supplied transcript text is shown as official feed text when present. Otherwise, set `OPENAI_API_KEY` on the local server to enable on-demand or automatic transcription of received TeamRadio clips. The browser sends only the F1 static session/clip path to the local server; the server fetches the audio, transcribes it with `OPENAI_TRANSCRIBE_MODEL` (default `gpt-transcribe`), caches the result for the lifetime of the server process, and returns only transcript text/model metadata to the UI. AI-generated transcripts are labelled separately because noisy radio audio can be misheard.

The transport is unofficial and Formula 1 can change, withhold or rename feeds without notice. Every extended feed is optional: missing or unrecognized data degrades to an unavailable/empty state instead of being fabricated.
...
The primary replay workflow is one-click **SYNC** at the current video position. The terminal reads the current F1 TV playback time, subtracts the known pre-race offset for that full-race asset, jumps the replay data to the corresponding race elapsed time, and then follows later F1 TV seeks. For example, if F1 TV is at 10:16 and the race-start offset is 9:11.3, SYNC places the data about 1:04.7 into the race. The anchor then follows the raw media clock so later UI-label quirks do not move the calibration.


When an F1 TV video source appears, the terminal attempts **automatic replay alignment**. The companion first reads Bitmovin's visible **current playback time** label, explicitly rejecting the right-side/total-duration label, then falls back to broader player UI metadata and finally the raw HTML5 media clock. This is necessary because the protected player can use a different MediaSource timestamp origin from the timeline F1 TV shows the viewer. It then asks the browser media element for the real-world start date carried by the stream and derives the UTC time of the frame currently on screen. That frame UTC maps directly onto OpenF1 timestamps, so arbitrary F1 TV seeks can locate the race without selecting a lap. If the browser/player does not expose a usable UTC clock, the terminal falls back to the selected session's public, curated F1 TV race-start offset from MultiViewer. No F1 TV URL, account data, cookies, video, audio, screenshot, or DRM information is sent to the lookup service; only the public OpenF1 meeting/session identifiers are used. Successful auto-sync enables **VIDEO LOCK**: F1 TV becomes the source of truth, and pause, seek/scrub, speed changes and stalls drive the terminal immediately. Manual terminal seek/play/rate controls remain disabled until VIDEO LOCK is turned off.

DETAILS opens the full synchronization panel. MATCH NOW remains the lap-based fallback when automatic metadata is unavailable or a particular replay has been edited differently. Fine offset buttons preserve VIDEO LOCK. Turning VIDEO LOCK off restores manual replay controls without discarding the anchor. Clearing an automatic anchor does not immediately recreate it for the same selected video source. Reloading/changing the race or selecting a new video source allows automatic matching again.

Browser companion **0.4.6** also offers **PIN VIDEO** in the actual frame that owns the selected F1 TV player when the browser exposes Picture-in-Picture. This keeps the original F1 TV video element and DRM playback in F1 TV while floating the browser-owned video window over the replay terminal. Google Cast is not used: Cast senders launch a receiver application by receiver app ID, so F1 TV's Cast session cannot be redirected into an arbitrary localhost panel.
...
- `lib/live-timing.ts` and `/api/live-timing`: pure live-feed reducer/selectors plus a Node-only SignalR-to-SSE bridge for Formula 1's anonymous timing stream. Compressed CarData.z and Position.z are inflated server-side and forwarded only to the local browser. Position history is accumulated client-side for the live track and is capped rather than retained indefinitely.
