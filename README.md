...
The live UI is driver-centric: selecting a car carries its team colour and identity through timing, telemetry, track position, strategy/activity, race-control mentions and team radio. The **Race Story** panel derives explainable factual context from received data—such as pace versus personal best, catching/battle state, tyre age, speed-trap rank and position trend—while session statistic tiles surface fastest lap, top speed, closest interval and oldest stint. F1-supplied championship projections are identified as projections rather than treated as results. A feed-coverage panel shows which subscribed topics have actually arrived so unavailable data is visible instead of silently inferred. Raw car-data channel 45 remains labelled **Aero ch45** because its meaning varies by ruleset and feed version.
