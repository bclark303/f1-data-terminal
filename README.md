...
The default replay workflow is now deliberately simple: open the terminal, start the matching F1 TV replay, and **scrub the video to the start of the race**. When the player enters the curated race-start window, the terminal automatically creates the anchor and changes the replay-bar control to **SYNCED**. From that point onward, F1 TV is the master clock and arbitrary seeks/scrubs drive the data terminal without another button press.
