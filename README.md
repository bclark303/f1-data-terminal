...
The default replay workflow supports a deterministic manual calibration path: start the matching F1 TV replay, scrub to the instant the race starts, and press **SYNC START** in the replay bar. That snapshots the raw HTML media clock at that exact instant and declares it equal to race start. From then on, F1 TV is the master clock and subsequent seeks/scrubs drive the terminal by raw clock delta, independent of F1 TV's displayed timestamp, Bitmovin UI labels, or curated start-offset metadata.
