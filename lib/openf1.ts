...
async function scheduledFetch(url: URL): Promise<Response> {
  ...
  const task = requestQueue.then(async () => {
    for (let attempt = 0; attempt <= MAX_429_RETRIES; attempt += 1) {
      ...
      if (response.status !== 429 || attempt === MAX_429_RETRIES) return;
...
async function query<T>(endpoint: string, params: Record<string, string | number>): Promise<T[]> {
  ...
  const pending = (async () => {
    ...
    if (!response.ok) {
      ...
      throw new Error(`OpenF1 ${endpoint} request failed: ${response.status}${detail ? ` — ${detail}` : ""}`);
    }
...
export const openF1 = {
  ...
  raceControl: (sessionKey: number) => query<RaceControlMessage>("race_control", { session_key: sessionKey }),
  ...
  carDataDriver: (sessionKey: number, driverNumber: number) =>
    query<CarDataPoint>("car_data", { session_key: sessionKey, driver_number: driverNumber }),
  locationDriver: (sessionKey: number, driverNumber: number) =>
    query<LocationPoint>("location", { session_key: sessionKey, driver_number: driverNumber }),
};
